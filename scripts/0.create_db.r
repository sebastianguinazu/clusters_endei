...
endei = endei %>% 
  ...
  mutate(dvinc.firmas = ifelse(p.9.1.b == 1 | p.9.2.b == 1 | p.9.3.b == 1 | p.9.4.b == 1 | p.9.5.b == 1 | p.9.6.b == 1 | p.9.7.b == 1,1,0),
         dvinc.pub    = ifelse(p.9.1.c == 1 | p.9.2.c == 1 | p.9.3.c == 1 | p.9.4.c == 1 | p.9.5.c == 1 | p.9.6.c == 1 | p.9.7.c == 1 | p.9.1.d == 1 | p.9.2.d == 1 | p.9.3.d == 1 | p.9.4.d == 1 | p.9.5.d == 1 | p.9.6.d == 1 | p.9.7.d == 1,1,0)) %>%
  rowwise() %>%
  mutate(svinc.firmas = sum(c(p.9.1.b, p.9.2.b, p.9.3.b, p.9.4.b, p.9.5.b, p.9.6.b, p.9.7.b)),
         svinc.pub    = sum(c(p.9.1.c, p.9.2.c, p.9.3.c, p.9.4.c, p.9.5.c, p.9.6.c, p.9.7.c, p.9.1.d, p.9.2.d,p.9.3.d, p.9.4.d, p.9.5.d, p.9.6.d, p.9.7.d, p.9.1.f, p.9.2.f, p.9.3.f, p.9.4.f, p.9.5.f, p.9.6.f, p.9.7.f)))
...
endei = endei %>% 
  mutate(k.inac = ifelse(p.1.8 == 'Con presencia de capital internacional', 1, 0),
         depto.id = ifelse(p.5.1.6 == 'Si',1,0)) 
...
# dnorm.calidad: Implementa Normas ISO o Normas sectoriales o de prodcutos
# d.especific:
# d.trazabilidad: 
# d.problemas:
# d.mejoracont:
...
# Politica de gestion del empleo (planes de carrera, incentivos, etc.) ------------------

# d.incent: aplica la firma algun sistema de evaluacion de desempeno
endei = endei %>% 
...
# dcap.cursos: si atendio alguna temÃ¡tica en los cursos(capta si hizo cursos/ojo: antes estÃ¡ pregunta de % de personal capacitado)
...
endei = endei %>%
  mutate_at(vars(starts_with("p.10.15.")),funs(case_when(. =="Implementa, planificada"~ 1, TRUE ~ 0)))  %>%
  rowwise() %>%
  mutate(d.rotacion = max(c(p.10.15.a,p.10.15.b,p.10.15.c)))


# Sectores de actividad y variables a dolares -------------------------------------------
